...
function toRings(rawValue) {
  ...
  if (Array.isArray(first) && first.length === 2 && first.every((v) => typeof v === 'number')) {
    return [
      rawValue.map((pair) => [roundCoord(pair[0]), roundCoord(pair[1])]).filter((pair) => pair.length === 2),
    ]
  }

  if (Array.isArray(first) && Array.isArray(first[0])) {
    return rawValue.map((ring) =>
      ring.map((pair) => [roundCoord(pair[0]), roundCoord(pair[1])]).filter((pair) => pair.length === 2),
    )
  }
