...
describe('computeFadeOpacity', () => {
  test('is 1 at alarm time', () => {
    expect(computeFadeOpacity({ nowMs: 1_000, alarmAtMs: 1_000, fadeMinutes: 60 })).toBe(1);
  });

  test('linearly fades to 0 at fade duration', () => {
    expect(computeFadeOpacity({ nowMs: 3_600_000, alarmAtMs: 0, fadeMinutes: 60 })).toBe(0);
  });

  test('never goes below 0', () => {
    expect(computeFadeOpacity({ nowMs: 9_999_999, alarmAtMs: 0, fadeMinutes: 1 })).toBe(0);
  });
});
